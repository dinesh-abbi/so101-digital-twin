...
6. RELATIVE TRACKING: the leader->follower offset is captured once at
   startup and subtracted from every command, so wherever the two arms
   happen to be when this starts counts as "matched" and the follower
   mirrors the leader's MOTION from there. The two arms never need to be
   hand-matched first. See the offset block in main() for why absolute
   tracking was abandoned.
...
# Same scene M7 uses -- the one whose meshdir resolves regardless of cwd.
...
def main():
    ...
    try:
        ...
        print("\n  Follower starting pose (normalised units):")
        outside = []
        for j in JOINT_ORDER:
            flag = "  <- LIVE" if j in live else ""
            warn = ""
            if is_outside(j):
                warn = "   *** OUTSIDE SAFE ENVELOPE ***"
                outside.append(j)
        ...
        if outside:
            ...
            print("  Recover first (torque stays on throughout):")
            print(f"    python m6_keyboard_real.py --port {args.follower_port} "
                  f"--id {args.follower_id} --joints {','.join(outside)} --recover")
            return

        # ---- Capture the leader->follower offset -----------------------
        # RELATIVE tracking, not absolute. Leader and follower are
        # calibrated independently, so equal normalised values are NOT the
        # same physical pose -- and demanding the two arms be hand-matched
        # before every session (the original design) turned out to be a
        # fiddly, drift-prone chore on real hardware: adjusting one joint
        # by hand nudges its neighbours, so the gaps kept moving between
        # checks.
        #
        # Instead, capture the gap ONCE, right now, and treat wherever the
        # two arms happen to be as "matched". From here the follower
        # mirrors the leader's MOTION rather than its absolute reading:
        #
        #     follower_target = leader_now - offset
        #
        # Consequence worth knowing: a large offset shifts which part of
        # the follower's +/-SAFE_LIMIT envelope your leader's comfortable
        # range maps onto, so the follower can hit its clamp earlier on one
        # side than the other. The readout below prints each offset so a
        # lopsided one is visible before you start moving.
        leader_action = leader.get_action()
        leader_start = {j: float(leader_action[f"{j}.pos"]) for j in JOINT_ORDER}
        offset = {j: leader_start[j] - start[j] for j in JOINT_ORDER}

        print("\n  Offset captured (leader - follower, at this instant):")
        for j in live:
            print(f"    {j:15s} leader{leader_start[j]:+7.1f}  "
                  f"follower{start[j]:+7.1f}  offset{offset[j]:+7.1f}")
        print("\n  Both arms are now treated as matched WHERE THEY ARE.")
        print("  Move the leader and the follower mirrors that movement.")
        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            while viewer.is_running():
                ...
                for j in live:
                    # Relative tracking: subtract the offset captured at
                    # startup, so the follower mirrors the leader's MOTION
                    # from where each arm began, not its absolute reading.
                    v = leader_pos[j] - offset[j]
                    if j == "gripper":
                ...
                if tick - last_print > 0.5:
                    # "want" is the offset-corrected target, not the
                    # leader's raw reading -- printing the raw value here
                    # would look like the follower was failing to track a
                    # number it is deliberately not chasing.
                    cells = []
                    for j in live:
